...
from templatetags import set_time_zone
import math
import re
import settings
import subprocess
...
def parse_regulatory_rule(equation, all_obj_data, species_wid):
	from helpers import getModel, getEntry
	import settings
...
class Molecule(SpeciesComponent):
	...
	def get_empirical_formula(self):
		from helpers import EmpiricalFormula
...
class Chromosome(Molecule):
	...
	def get_extinction_coefficient(self):	
		from helpers import ExtinctionCoefficient
	...
	def get_as_html_sequence(self, is_user_anonymous):
		from helpers import format_sequence_as_html
...
class Metabolite(Molecule):
	...
	def calculate_properties(self):	
		import sys	
		subprocess.call('cxcalc name -t preferred name -t traditional logP logD -H 7.5 formalcharge -H 7.5 isoelectricpoint volume "%s"' % (settings.BASE_DIR, self.smiles, ))
	...
	#html formatting
	def get_as_html_structure(self, is_user_anonymous):
		from helpers import draw_molecule
...
class Process(SpeciesComponent):
	...
	class Meta:		
		...
		def validate_unique(self, model_objects_data, all_obj_data=None, all_obj_data_by_model=None):
			order = []
...
class ProteinComplex(Protein):
	...
	def get_empirical_formula(self):
		from helpers import EmpiricalFormula, getModel
...
class Reaction(SpeciesComponent):
	...
	class Meta:
		...
		def clean(self, obj_data, all_obj_data=None, all_obj_data_by_model=None):
			from helpers import getEntry, getModel, EmpiricalFormula
